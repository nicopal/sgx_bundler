...
#define _DO_WHILE(SM_CTX, ID, CONFIG, CONDITION_ARGS, BODY_ARGS) \
    ...
    struct hotcall_do_while ID = { CAT2(DO_WHILE_BODY_ARG_,ID), &CAT2(DO_WHILE_CONFIG_,ID) };\
    struct ecall_queue_item CAT2(QUEUE_ITEM_, ID) = { QUEUE_ITEM_TYPE_DO_WHILE, .call = { .dw = &ID }}; \
    ...
    CAT2(DO_WHILE_CONFIG_,ID).postfix_length = to_postfix(CAT2(DO_WHILE_CONFIG_,ID).predicate_fmt, CAT2(DO_WHILE_CONFIG_,ID).condition_params, CAT2(DO_WHILE_CONFIG_,ID).postfix);
